...
class SC2LotVLogic(LogicMixin):
    ...
    def _sc2lotv_has_air(self, multiworld: MultiWorld, player: int) -> bool:
        return self.has_any({'Phoenix', 'Void Ray', 'Carrier'}, player) or get_option_value(multiworld, player,
                                                                                            'required_tactics') > 0 \

    def _sc2lotv_has_anti_air(self, multiworld: MultiWorld, player: int) -> bool:
        return self.has_any({'Stalker', 'Sentry', 'Phoenix', 'Void Ray', "Carrier"}, player) \
            or get_option_value(multiworld, player, 'required_tactics') > 0 and self.has_any({'High Templar'}, player)


    # def _sc2lotv_has_good_antiair(self, multiworld: MultiWorld, player: int) -> bool:
    #     return self.has_any({'Stalker'}, player)
    ...
    # def _sc2lotv_has_minimal_antiair(self, multiworld: MultiWorld, player: int) -> bool:
    #        return self._sc2lotv_has_good_antiair(multiworld, player) or get_option_value(multiworld, player, 'kerriganless') == 0 or \
    #       self.has('photon cannon', player) or (get_option_value(multiworld, player, 'required_tactics') > 0 and self.has('Khaydarin monolith', player))

    #    def _sc2lotv_has_brood_lord(self, multiworld: MultiWorld, player: int) -> bool:
    #        return self.has_all({'Mutalisk', 'Brood Lord Strain (Mutalisk)'}, player)

    #    def _sc2lotv_has_viper(self, multiworld: MultiWorld, player: int) -> bool:
    #        return self.has_all({'Mutalisk', 'Viper Strain (Mutalisk)'}, player)

    #    def _sc2lotv_has_impaler_or_lurker(self, multiworld: MultiWorld, player: int) -> bool:
    ...
    #  core_unit = self.has_any({'Zealot', 'Stalker'}, player)
    #     support_unit = self.has_any({'Sentry', 'High Templar'}, player) \
    #                    or self._sc2lotv_has_brood_lord(multiworld, player) \
    #                    or advanced and (self.has('Infestor', player) or self._sc2lotv_has_viper(multiworld, player))
    #     if core_unit
    #         return True
    #  vespene_unit = self.has_any({'Immortal', 'Dark Templar'}, player) \
    #                    or advanced and self._sc2lotv_has_viper(multiworld, player)
    #  return vespene_unit and self.has_any({'Zealot', 'Stalker'}, player)

    # def _sc2lotv_can_spread_creep(self, multiworld: MultiWorld, player: int) -> bool:
    #     return get_option_value(multiworld, player, 'required_tactics') > 0 or self.has('Swarm Queen', player)

    # def _sc2lotv_has_competent_defense(self, multiworld: MultiWorld, player: int) -> bool:
    #    return self._sc2lotv_has_common_unit(multiworld, player) \
    #         ((self.has('Swarm Host', player) or self._sc2lotv_has_brood_lord(multiworld, player) or self._sc2lotv_has_impaler_or_lurker(multiworld, player)) or \
    #         (get_option_value(multiworld, player, 'required_tactics') > 0 and (self._sc2lotv_has_viper(multiworld, player) or self.has('Spine Crawler', player))))

    # def _sc2lotv_has_basic_kerrigan(self, multiworld: MultiWorld, player: int) -> bool:
    ...
    # def _sc2lotv_has_two_kerrigan_actives(self, multiworld: MultiWorld, player: int) -> bool:
    #    count = 0
    #    for i in range(7):
    ...
    # def _sc2lotv_has_low_tech(self, multiworld: MultiWorld, player: int) -> bool:
    #    return self.has_any({'Zealot', 'Stalker', 'Photon Cannon'}, player) \
    #            or self._sc2lotv_has_common_unit(multiworld, player) and (
    #                   self._sc2lotv_has_basic_kerrigan(multiworld, player)
    #                    get_option_value(multiworld, player, 'required_tactics') > 0
    #           )
