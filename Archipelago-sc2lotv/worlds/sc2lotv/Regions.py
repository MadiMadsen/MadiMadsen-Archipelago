...
#                lambda state: state.has("Beat Death From Above", player)),
        ...
        return vanilla_mission_req_table, 22, 'Salvation: Victory'
        ...
        def build_connection_rule(mission_names: List[str], missions_req: int) -> Callable:
            if len(mission_names) > 1:
                return lambda state: state.has_all({f"Beat {name}" for name in mission_names}, player) and \
                    state._sc2lotv_cleared_missions(multiworld, player, missions_req)
            else:
        ...
        #         continue
        #     connections = []
        #     if mission_order[i].or_requirements:
        #         for connection in mission_order[i].connect_to:
        #             required_mission = missions[connection]
        #             if connection == -1:
        #                 connect(multiworld, player, names, "Menu", mission)
        ...
        #             required_mission = missions[connection]
        #             if connection == -1:
        #                 requirements.add("Menu")
        #             elif required_mission is None:
        #                 continue
        #             else:
        #                 requirements.add(f"Beat {required_mission}")
        #         if len(requirements) > 0:
        #             any_required_mission = mission_order[i].connect_to[0]
        #             connect(multiworld, player, names, missions[any_required_mission], mission,
        #                     (lambda names, missions_req: (lambda state: state.has_all(names, player) and
        #                                                                 state._sc2hots_cleared_missions(multiworld, player,
        #                                                                                                missions_req)))
        #                     (requirements, mission_order[i].number))
        #             connections.append(slot_map[any_required_mission])
...
#          final_location = 'The Reckoning: Victory'
        ...
        if final_mission != 'Salvation':
            ...
            final_location = 'Salvation: Victory'
