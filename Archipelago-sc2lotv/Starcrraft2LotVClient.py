...
from worlds.sc2lotv import SC2LotVWorld
from worlds.sc2lotv.Items import lookup_id_to_name, item_table, ItemData, type_flaggroups
from worlds.sc2lotv.Locations import SC2LOTV_LOC_ID_OFFSET
from worlds.sc2lotv.MissionTables import lookup_id_to_mission
from worlds.sc2lotv.Regions import MissionInfo
...
class SC2Context(CommonContext):
    ...
    def run_gui(self):
        ...
        import pkgutil
        data = pkgutil.get_data(SC2LotVWorld.__module__, "Starcraft2.kv").decode()
    ...
    def build_location_to_mission_mapping(self):
        ...
        for loc in self.server_locations:
            mission_id, objective = divmod(loc - SC2LOTV_LOC_ID_OFFSET, victory_modulo)
    ...
    def locations_for_mission(self, mission: str):
        ...
        for objective in objectives:
            yield SC2LOTV_LOC_ID_OFFSET + mission_id * 100 + objective
                ...
                if self.can_read_game:
                    if game_state & (1 << 1) and not self.mission_completed:
                        if self.mission_id != self.ctx.final_mission:
                            ...
                            await self.ctx.send_msgs(
                                [{"cmd": 'LocationChecks',
                                  "locations": [SC2LOTV_LOC_ID_OFFSET + victory_modulo * self.mission_id]}])
                    ...
                    for x, completed in enumerate(self.boni):
                        if not completed and game_state & (1 << (x + 2)):
                            await self.ctx.send_msgs(
                                [{"cmd": 'LocationChecks',
                                  "locations": [SC2LOTV_LOC_ID_OFFSET + victory_modulo * self.mission_id + x + 1]}])
                            self.boni[x] = True
    ...
    if len(ctx.mission_req_table[mission_name].required_world) >= 1:
        ...
        for req_mission in ctx.mission_req_table[mission_name].required_world:
            ...
            if not (ctx.mission_req_table[list(ctx.mission_req_table)[req_mission - 1]].id *
                    victory_modulo + SC2LOTV_LOC_ID_OFFSET) in ctx.checked_locations:
...
def is_mod_installed_correctly() -> bool:
    ...
    # Check for maps.
    missing_maps = []
    for mapfile in lotv_required_maps:
